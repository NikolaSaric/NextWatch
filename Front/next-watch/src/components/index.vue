<template>
  <div>
    ...
    <v-layout align-start row justify-start class="UpperLowerMargin5">
        ...
        <v-btn @click="newPrevious"><v-icon>arrow_left</v-icon></v-btn>
        <v-btn @click="newNext"><v-icon>arrow_right</v-icon></v-btn>
        <v-btn>More</v-btn>
    </v-layout>

    <v-layout row>
      <v-flex xs6 order-lg2 v-for="newMovie in newestShow" :key="newMovie.id">
          <moviePreview :rating=newMovie.imdbRating :filePath=newMovie.Poster :movieName=newMovie.Title ></moviePreview>
        </v-flex>
